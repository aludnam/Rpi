...
Nc=12*24*5 	#number of cycles
Nt=1 		#number of acquisition within each cycle
wait_time_s = 5*60 	#time between the cycles in s
exposure = 12 		#exposure time in ms
...
while n<Nc:
	...
	m, s = divmod(wait, 60)
	h, m = divmod(m, 60)	
	print('Waiting %d:%02d:%02d (h:m:s).'%(h, m, s))
	print('Press CTRL+c to stop the measurement.')
	#raw_input('Press ENTER to continue.')
	time.sleep(wait)
	
raw_input('Acquisition finished. Unplug LED and press ENTER.')
p.stop()
GPIO.cleanup()
